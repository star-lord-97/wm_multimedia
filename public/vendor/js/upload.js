function ProgressTracker(element, opts) {
    ...
    this.update = function (info) {
        ...
        $(element)
            ...
            .css("width", Math.round(rate * $(".progressbar-outer").width()));
        $(completed).html(
            "<b>" + convertSize(info.loaded) + "</b> of <b>" + convertSize(info.total) + "</b>"
        );
...
function FileUploader(f1, opts) {
    this.start = function (files) {
        ...
        var tracker = new ProgressTracker($(f1).find(".progress_div"), { filenames: filenames });
...
function formToXHR(form, url) {
    ...
    xhr.onerror = function (evt) {
        if (document.location.protocol == "https:" && !post_url.match(/^https:/)) {
            alert(
                "Plain HTTP uploads are prohibited on HTTPS site by browser's security policies.\n\n" +
                    "Please use your Admin Servers interface in order to switch file servers to HTTPS."
            );
        } else if (document.location.protocol == "https:" && !evt.target.status) {
            alert(
                "HTTPS upload was denied by browser.\n\n" +
                    "Most likely your file server's certificate had expired, or you're using a self-signed certificate."
            );
        } else {
